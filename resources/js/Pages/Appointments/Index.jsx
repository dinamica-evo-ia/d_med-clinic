...
export default function Index() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setEvents((data||[]).map(e => ({ ...e, _s: parse(e.start), _e: parse(e.end || e.start) })));
    } catch { setEvents([]); }
    setLoading(false);
  }, [range]);
  useEffect(() => { load(); }, [load]);
    ...
    <div className="flex flex-col h-[calc(100vh-7rem)]">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <button onClick={()=>setCursor(startOfDay(new Date()))} className="px-3 h-9 rounded-lg border border-slate-200 text-sm font-medium text-slate-700 hover:bg-slate-50">Hoje</button>
          <button onClick={()=>go(1)} className="w-9 h-9 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50">›</button>
      ...
      <div className="flex-1 overflow-hidden rounded-2xl border border-slate-200 bg-white">
        {view==='month' && <MonthView cursor={cursor} range={range} events={events} today={today} onDay={(d)=>{setCursor(d);setView('day');}} />}
        {view==='week' && <TimeGrid days={Array.from({length:7},(_,i)=>addDays(range[0],i))} events={events} today={today} now={nowTick} />}
        {view==='day' && <TimeGrid days={[startOfDay(cursor)]} events={events} today={today} now={nowTick} single />}
      </div>
    </div>
  );
}

function MonthView({ cursor, range, events, today, onDay }) {
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="grid grid-cols-7 grid-rows-6 flex-1">
        {days.map((d,i)=>{
          ...
          return (
            <div key={i} className={`border-b border-r border-slate-100 p-1 overflow-hidden ${out?'bg-slate-50/60':''}`}>
              ...
              <div className="mt-0.5 space-y-0.5">
                {evs.slice(0,3).map(ev=>(
                  <button key={ev.id} onClick={()=>open(ev)} title={ev.title}
...
function layout(evs) {
  ...
  sorted.forEach(ev=>{
    let placed=false;
    for (let c=0;c<cols.length;c++){ if (cols[c][cols[c].length-1]._e<=ev._s){ cols[c].push(ev); ev._col=c; placed=true; break; } }
...
function TimeGrid({ days, events, today, now, single }) {
          ...
          {days.map((d,di)=>{
            ...
            return (
              <div key={di} className="border-l border-slate-100">
                <div className={`h-10 sticky top-0 z-10 bg-white border-b border-slate-200 flex flex-col items-center justify-center ${single?'':''}`}>
                ...
                <div className="relative" style={{height:totalH}}>
                  ...
                  {isToday && now.getHours()>=H0 && now.getHours()<=H1 && (
                    <div className="absolute left-0 right-0 z-20" style={{top:pos(d,now)}}>
                  ...
                  {evs.map(ev=>{
                    ...
                    return (
                      <button key={ev.id} onClick={()=>open(ev)} title={ev.title}
                        className="absolute rounded-lg px-2 py-1 text-left text-white overflow-hidden shadow-sm"
